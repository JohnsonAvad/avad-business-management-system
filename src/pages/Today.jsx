...
export default function Today() {
  ...
  const briefLines = [
    ...
    '',
    '💰 Yesterday: ' + format(yRevenue) + ' sold • ' + format(yProfit) + ' profit',
    '🔴 Owed to you: ' + format(totalOwed) + (debtors.length ? ' (' + debtors.length + ' customer' + (debtors.length > 1 ? 's' : '') + ')' : ''),
  ];
  if (restocks.length) briefLines.push('📦 Restock soon: ' + restocks.slice(0, 3).map(p => p.name).join(', '));
  ...
  return (
    <>
      ...
      <div className="stat-grid">
        <div className="stat-card"><div><div className="stat-label">Yesterday's Sales</div><div className="stat-value blue">{format(yRevenue)}</div></div></div>
